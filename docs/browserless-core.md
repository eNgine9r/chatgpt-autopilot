...
## Project-level GitHub quiet window
The GitHub debounce window applies to the whole project, not individual events. If any pending GitHub observation for a project is newer than the configured quiet cutoff, none of that project's GitHub observations are queued yet. Once the whole project has been quiet long enough, latest-per-subject coalescing runs first and only then is the resulting single event or bounded batch queued. Operator, bootstrap, and action-evidence events remain immediate.
## Reused suppression continuation
If a read returns unchanged evidence whose suppression observation already exists from an earlier workflow, Browserless emits one action-scoped `suppression_reused` continuation event instead of leaving an active checkpoint with an empty queue. The event carries only the action type, target, and result hash. A subsequent request for the same type/target is blocked by the existing `repeated_suppressed_action` core guard before another external read; a distinct action, wait, complete, or escalation remains allowed.
## Restart quiescence
Supervisor startup does not synthesize a `bootstrap.resume` event for an active project when the latest completed decision is `wait`, `user_action_required`, or `escalation_required`. It also skips bootstrap when that project already has unfinished durable work: pending/queued events, pending/running jobs, or planned/running actions. Fresh GitHub/runtime/operator events remain authoritative and are queued normally by the scheduler after restart. This preserves the invariant: no new event means no Luna call.
## Per-project action drain
A project cannot claim a new Luna job while any action request for that same project is `planned` or `running`. The action queue must fully drain before the next AI decision for that project. Other projects remain eligible, so one project waiting on safe-tool execution does not block unrelated work. This prevents stale evidence jobs from planning duplicate reads while earlier actions are still pending.
