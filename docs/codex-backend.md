...
- `[[AUTOPILOT_PUBLISH]]` — source/tests are ready, but Git metadata is sandbox-protected; hand the tracked diff to the deterministic restricted publisher, then re-evaluate canonical Work Package/branch state before any further turn.
...
When `codex.publisher.enabled` is true, Codex never receives write access to `.git`. Before each reasoning turn the publisher records a clean tracked-worktree baseline containing the exact HEAD, current branch, and (when present) canonical `.project/ACTIVE_SPRINT.json` active Work Package. A turn is refused before model execution if the canonical active branch already differs from the checkout. `[[AUTOPILOT_PUBLISH]]` succeeds only if the exact HEAD and checkout still match the pre-turn baseline, the baseline branch is not `main`/`master`, `git diff --check` passes, and no non-cache untracked source is present. This deliberately allows a verified turn to reconcile `.project/ACTIVE_SPRINT.json` without weakening Git safety: Codex cannot change the checked-out branch because `.git` remains sandbox-protected.

After the restricted gateway creates one non-GPG commit and pushes `HEAD` to that exact baseline feature branch without force or merge, the backend performs a fresh restricted `inspect`. It continues only when the post-publish checkout is clean and the canonical active Work Package still names that same branch. If the active Work Package becomes `null`, the autonomous cycle completes. If canonical state selects another branch, auto-continuation pauses before another model turn; a later deterministic branch-alignment mechanism or operator action must establish that checkout first. This prevents next-Work-Package edits from being committed to the previous Work Package branch.
