...
When `codex.publisher.enabled` is true, Codex never receives write access to `.git`. Before each reasoning turn the publisher records a clean tracked-worktree baseline containing the exact HEAD and current branch. `[[AUTOPILOT_PUBLISH]]` succeeds only if both still match that baseline, the baseline branch is not `main`/`master`, `git diff --check` passes, and no non-cache untracked source is present. This deliberately allows a verified turn to reconcile `.project/ACTIVE_SPRINT.json` (including completing or switching the active Work Package) without weakening Git safety: Codex cannot change the checked-out branch because `.git` remains sandbox-protected. The restricted gateway stages tracked changes only, creates one non-GPG commit, and pushes `HEAD` to that exact baseline feature branch without force or merge.
