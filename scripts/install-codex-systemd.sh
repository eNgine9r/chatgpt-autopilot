...
SERVICE="$SERVICE_DIR/chatgpt-codex-autopilot.service"
...
mkdir -p "$SERVICE_DIR"
TMP="$(mktemp "$SERVICE_DIR/.autopilot-codex-service.XXXXXX")"
trap 'rm -f "$TMP"' EXIT
sed \
  -e "s|__HOME__|$HOME_DIR|g" \
  -e "s|__APP_DIR__|$APP_DIR|g" \
  -e "s|__NODE__|$NODE_BIN|g" \
  "$APP_DIR/systemd/chatgpt-codex-autopilot.service.template" > "$TMP"
chmod 0644 "$TMP"
mv "$TMP" "$SERVICE"
trap - EXIT
...
systemctl --user enable --now chatgpt-codex-autopilot.service
...
systemctl --user --no-pager --full status chatgpt-codex-autopilot.service || true
