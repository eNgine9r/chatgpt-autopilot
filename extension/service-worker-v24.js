importScripts("bridge-policy.js", "lease-policy.js", "discovery-policy.js", "mirror-sync-policy.js", "recovery-policy.js", "rollover-policy.js", "navigation-pressure-policy.js", "maintenance-policy.js", "discovery-runtime-policy.js", "source-tab-policy.js");
...
const DISCOVERY_DIAGNOSTIC_PREFIX = "discovery-diagnostic:";
...
const NAVIGATION_MIN_GAP_MS = 90000;
const claimChains = new Map();
const MAINTENANCE_STATE_KEY = "maintenance:state";
...
async function readMaintenanceState() {
  try {
    const value = (await chrome.storage.session.get(MAINTENANCE_STATE_KEY))[MAINTENANCE_STATE_KEY] || {};
    return {
      lastStartedAt: Number(value.lastStartedAt || 0),
      lastCompletedAt: Number(value.lastCompletedAt || 0),
      lastSource: String(value.lastSource || ""),
      consecutiveFailures: Number(value.consecutiveFailures || 0)
    };
  } catch {
    return { lastStartedAt: 0, lastCompletedAt: 0, lastSource: "", consecutiveFailures: 0 };
  }
}

async function writeMaintenanceState(patch = {}) {
  const current = await readMaintenanceState();
  const next = { ...current, ...patch };
  await chrome.storage.session.set({ [MAINTENANCE_STATE_KEY]: next });
  return next;
}

async function maintenanceSnapshot() {
  return AutopilotMaintenancePolicy.snapshot({ ...(await readMaintenanceState()), running: maintenanceRunning });
}

async function runMaintenance({ source = "unknown", sendPulse = false, force = false } = {}) {
  let failed = false;
  try { await ensurePulse(); }
  catch { failed = true; }

  const durable = await readMaintenanceState();
  const now = Date.now();
  if (!AutopilotMaintenancePolicy.shouldRun({
    running: maintenanceRunning,
    lastStartedAt: durable.lastStartedAt,
    now,
    force
  })) return { ok: !failed, ran: false };

  maintenanceRunning = true;
  try {
    try { await writeMaintenanceState({ lastStartedAt: now, lastSource: source }); }
    catch { failed = true; }
    if (sendPulse) {
      try {
        const tabs = await chrome.tabs.query({ url: "https://chatgpt.com/*" });
        await Promise.allSettled(tabs.filter((tab) => Number.isInteger(tab.id))
          .map((tab) => chrome.tabs.sendMessage(tab.id, { type: "PULSE" })));
      } catch { failed = true; }
    }
    const steps = [
      monitorConfiguredTabs,
      processRecoveries,
      processPendingRolloversSerial,
      processPendingDiscoveries,
      processPendingMirrorProbes,
      maybeStartDiscoveryScans,
      maybeStartMirrorProbe
    ];
    for (const step of steps) {
      try { await step(); }
      catch { failed = true; }
    }
    return { ok: !failed, ran: true };
  } finally {
    const latest = await readMaintenanceState();
    try {
      await writeMaintenanceState({
        lastCompletedAt: Date.now(),
        consecutiveFailures: failed ? Number(latest.consecutiveFailures || 0) + 1 : 0
      });
    } catch {}
...
async function maybeStartMirrorProbe() {
  ...
  for (const project of projects) {
    ...
    if (!status) continue;
    if (status.rateLimited) { await markRateLimitBackoff(); return; }
...
async function startRecovery(project, { tab = null, reason = "unknown", status = null } = {}) {
  ...
  if (status?.rateLimited) {
    ...
    return recoveryFailed(project.id, reason, "rate_limited_backoff");
  }
...
async function readDiscoveryDiagnostic(projectId) {
  const key = `${DISCOVERY_DIAGNOSTIC_PREFIX}${projectId}`;
  try {
    const value = (await chrome.storage.session.get(key))[key] || {};
    return AutopilotDiscoveryRuntimePolicy.state(value);
  } catch {
    return AutopilotDiscoveryRuntimePolicy.state();
  }
}

async function reportDiscoveryDiagnostic(projectId, patch = {}) {
  try {
    const key = `${DISCOVERY_DIAGNOSTIC_PREFIX}${projectId}`;
    const current = await readDiscoveryDiagnostic(projectId);
    const next = AutopilotDiscoveryRuntimePolicy.state({ ...current, ...patch, updatedAt: Date.now() });
...
async function discoveryDiagnosticSnapshot(projectId) {
  return AutopilotDiscoveryRuntimePolicy.snapshot(await readDiscoveryDiagnostic(projectId));
}
...
async function maybeStartDiscoveryScans() {
  const projects = await getProjects();
  const pendingEntries = await discoveryEntries();
  const pendingById = new Map(pendingEntries.map((entry) => [entry.projectId, entry]));
  const now = Date.now();
  for (const project of projects) {
    if (!project.chatDiscovery?.enabled || !project.projectRootUrl) continue;
    const lastKey = `${DISCOVERY_LAST_PREFIX}${project.id}`;
    const last = (await chrome.storage.session.get(lastKey))[lastKey] || {};
    const scanGeneration = Number(project.control?.discoveryScanGeneration || 0);
    const consumedGeneration = Number(last.scanGeneration || 0);
    const forced = scanGeneration > consumedGeneration;
    const diagnostic = {
      controlGeneration: scanGeneration,
      scanGeneration: consumedGeneration,
      pending: false
    };
    if (pendingById.has(project.id)) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "pending", pending: true });
      continue;
    }
    const due = now - Number(last.lastScanAt || 0) >= Number(project.chatDiscovery.intervalSeconds || 300) * 1000;
    const sourceTabs = await chrome.tabs.query({ url: "https://chatgpt.com/*" });
    const exactSourceTabs = sourceTabs.filter((tab) =>
      normalizeChatUrl(tab.url || "") === normalizeChatUrl(project.chatUrl || "")
    );
    if (!exactSourceTabs.length) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "no_source_tab" });
      continue;
    }
    const responsiveSource = await AutopilotSourceTabPolicy.firstResponsive({
      tabs: exactSourceTabs,
      target: normalizeChatUrl(project.chatUrl || ""),
      normalize: normalizeChatUrl,
      probe: tabRuntimeStatus
    });
    if (!responsiveSource) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "source_status_unavailable" });
      continue;
    }
    const sourceTab = responsiveSource.tab;
    const status = responsiveSource.status;
    if (status.rateLimited) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "rate_limited" });
      await markRateLimitBackoff();
      return;
    }
    if (!AutopilotDiscoveryPolicy.shouldStartScan({
      ...
      await reportDiscoveryDiagnostic(project.id, {
        ...diagnostic,
        gate: status.generating ? "generation_active" : "policy_wait"
      });
      continue;
    }
    if (!await claimBrowserNavigation({ paused: Boolean(project.control?.paused), forced })) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "navigation_denied" });
      return;
    }
    ...
    await chrome.storage.session.set({
      [key]: { tabId: tab.id, sourceTabId: sourceTab.id, startedAt: now, forced, scanGeneration },
      [lastKey]: { lastScanAt: now, scanGeneration }
    });
    await reportDiscoveryDiagnostic(project.id, {
      gate: "started", controlGeneration: scanGeneration, scanGeneration: scanGeneration, pending: true
    });
  }
}
...
async function processPendingDiscoveries() {
  ...
  for (const entry of entries) {
    ...
    const controlGeneration = Number(project?.control?.discoveryScanGeneration || 0);
    const scanGeneration = Number(entry.scanGeneration || 0);
    const diagnostic = { controlGeneration, scanGeneration, pending: true };
    if (!project?.chatDiscovery?.enabled || (project.control?.paused && !entry.forced)) {
      await reportDiscoveryDiagnostic(entry.projectId, { ...diagnostic, gate: "policy_wait", pending: false });
    ...
    if (AutopilotDiscoveryPolicy.scanDisposition({ timedOut: elapsedMs > DISCOVERY_TIMEOUT_MS }) === "timeout") {
      ...
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "timeout", pending: false });
      await closeDiscoveryEntry(entry);
      continue;
    }
    let tab;
    try {
      ...
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "pending_tab_missing", pending: false });
      await closeDiscoveryEntry(entry);
      continue;
    }
    if (tab.status !== "complete" || projectIdFromRoot(tab.url || "") !== projectIdFromRoot(project.projectRootUrl)) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "root_not_ready" });
      continue;
    }
    const discoveryStatus = await tabRuntimeStatus(entry.tabId);
    if (discoveryStatus?.rateLimited) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "rate_limited", pending: false });
      await markRateLimitBackoff();
      await closeDiscoveryEntry(entry);
      continue;
    }
    ...
    try {
      ...
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "scan_message_unavailable" });
      continue;
    }
    if (!scan?.ok || !Array.isArray(scan.candidates)) {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "scan_message_unavailable" });
      continue;
    }
    ...
    if (disposition !== "finalize") {
      await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "candidate_wait" });
      continue;
    }
    ...
    await reportDiscoveryDiagnostic(project.id, { ...diagnostic, gate: "finalized", pending: false });
    await closeDiscoveryEntry(entry);
  }
}
...
async function processRecoveries() {
  ...
  for (const project of projects) {
    ...
    if (status?.rateLimited) {
      await markRateLimitBackoff();
      if (stage !== "failed") await recoveryFailed(project.id, recovery.reason || "session_rate_limited", "rate_limited_backoff");
      continue;
    }
    if (status?.composerPresent) {
...
async function processPendingMirrorProbes() {
  ...
  for (const entry of entries) {
    ...
    if (["same", "blocked"].includes(disposition.action)) {
      if (snapshot.rateLimited) await markRateLimitBackoff();
      await mirrorReport(project.id, {
    ...
    if (disposition.action === "refresh") {
      ...
      if (!sourceStillSafe) {
        if (sourceSnapshot?.rateLimited) await markRateLimitBackoff();
        await mirrorReport(project.id, {
...
async function monitorConfiguredTabs() {
  ...
  for (const project of projects) {
    ...
    if (status?.rateLimited) {
      await markRateLimitBackoff();
      if (!project.control?.paused && recoveryEnabled) await recoveryFailed(project.id, "session_rate_limited", "rate_limited_backoff");
      continue;
    }
...
async function handleMessage(message, sender) {
  switch (message?.type) {
    ...
    case "HEARTBEAT": {
      const response = await heartbeatBridge(message.projectId, message.progressKey, message.status, {
        ...
        backgroundWorker: "v24",
        ...(await maintenanceSnapshot()),
        ...(await discoveryDiagnosticSnapshot(message.projectId))
      });
      runMaintenance({ source: "heartbeat" }).catch(() => {});
...
chrome.alarms.onAlarm.addListener((alarm) => {
  ...
  runMaintenance({ source: "alarm", sendPulse: true, force: true }).catch(() => {});
});

chrome.tabs.onRemoved.addListener((tabId) => {
  releaseLeasesForTab(tabId).catch(() => {});
  cleanupMirrorEntriesForTab(tabId).catch(() => {});
  runMaintenance({ source: "tab_removed", force: true }).catch(() => {});
});

chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
  if (changeInfo.url) releaseLeasesForTab(tabId).catch(() => {});
  if (changeInfo.url || changeInfo.status === "complete") {
    runMaintenance({ source: "tab_updated", force: true }).catch(() => {});
  }
});

function startMaintenance(source, { force = false } = {}) {
  runMaintenance({ source, force }).catch(() => {});
}

chrome.runtime.onInstalled.addListener(() => startMaintenance("installed", { force: true }));
chrome.runtime.onStartup.addListener(() => startMaintenance("startup", { force: true }));
startMaintenance("service_worker_start");
