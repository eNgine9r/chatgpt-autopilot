...
const maintenanceState = { running: false, lastStartedAt: 0, lastCompletedAt: 0, lastSource: "", consecutiveFailures: 0 };
...
async function runMaintenance({ source = "unknown", sendPulse = false, force = false } = {}) {
  ...
  if (!AutopilotMaintenancePolicy.shouldRun({
    ...
    lastStartedAt: maintenanceState.lastStartedAt,
    now,
    force
  })) return { ok: true, ran: false };

  maintenanceState.running = true;
  maintenanceState.lastStartedAt = now;
  maintenanceState.lastSource = source;
  let failed = false;
  try {
    ...
    maintenanceState.consecutiveFailures = failed ? maintenanceState.consecutiveFailures + 1 : 0;
    maintenanceState.lastCompletedAt = Date.now();
  ...
  switch (message?.type) {
    ...
    case "HEARTBEAT": {
      const response = await heartbeatBridge(message.projectId, message.progressKey, message.status, {
        ...
        backgroundWorker: "v21",
        ...AutopilotMaintenancePolicy.snapshot(maintenanceState)
      });
...
function startMaintenance(source) {
  ensurePulse()
    .then(() => runMaintenance({ source, force: true }))
    .catch(() => {});
}

chrome.runtime.onInstalled.addListener(() => startMaintenance("installed"));
chrome.runtime.onStartup.addListener(() => startMaintenance("startup"));
